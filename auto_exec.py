...
def _cancel_stop_and_confirm_absent(client, account_id_key: str, symbol: str, lifecycle, filled_qty: int | None = None, note_prefix: str = 'STOP_CANCEL') -> bool:
    """Best-effort cancel of the protective STOP, only clearing local ids when broker truth confirms absence.

    Returns True when the STOP is confirmed absent from broker OPEN orders (or there was no OPEN STOP to begin with).
    Returns False when the STOP still appears OPEN / cancel remains ambiguous.
    """
    try:
        open_orders = _extract_open_orders_for_symbol(client, account_id_key, symbol)
    except Exception:
    ...
    broker_oid = existing.get('order_id')
    if broker_oid not in (None, '', 0):
        lifecycle.stop_order_id = str(broker_oid)
    try:
        oid_int = int(broker_oid) if broker_oid not in (None, '', 0) else None
    except Exception:
        oid_int = None
    ...
    lifecycle.stop_order_id = str(broker_oid2) if broker_oid2 not in (None, '', 0) else lifecycle.stop_order_id
    _append_note(lifecycle, f"{note_prefix}_PENDING")
    return False
...
""",
        ...
        if bracket_qty > 0 and pos_qty is not None:
            ...
            if pos_qty == 0:
                ...
                _close(inferred or _pending_exit_reason("CLOSED_POSITION_ZERO_UNKNOWN"))
                return
                ...
                if dt is not None and (now - dt).total_seconds() >= 90:
                    ...
                    if rem > 0:
                        try:
                            ...
                            _log(f"[AUTOEXEC][BROKER] place_ok sym={lifecycle.symbol} lc={lifecycle.lifecycle_id} coid={mcoid} pid={mpid} oid={mid} leg=MKTP0")
                        except Exception:
                            pass
                    _append_note(lifecycle, "TP0_TRIGGER_TIMEOUT_MARKET")
                    _mark_exit_sent(lifecycle, now, "TP0_TRIGGER_TIMEOUT_MARKET", order_id=lifecycle.market_exit_order_id, mode="market_close")
                    return

            # If TP0 triggered but no TP0 order was successfully attached/placed, fail-safe flatten after 90s.
            if tp0_trig_ts and not lifecycle.tp0_order_id:
                try:
                    dt = datetime.fromisoformat(str(tp0_trig_ts))
                except Exception:
                    dt = None
                if dt is not None and (now - dt).total_seconds() >= 90:
                    ...
                    if rem > 0:
                        try:
                            mcoid = _mk_client_order_id(lifecycle.lifecycle_id, 'MKTP0N')
                            if _close_inflight(lifecycle, now, mcoid, 90):
                                _append_note(lifecycle, 'MK_TP0_NOORDER_SKIP_DUP')
                            else:
                                _mark_close_submit_started(lifecycle, now, mcoid)
                                mode, mid, mpid = _place_sell_close_best_effort(
                                    client=client,
                                    ...
                                    last_px=shared_last_px,
                                    lifecycle=lifecycle,
                                    note_prefix="MK_TP0_NOORDER",
                                )
                                lifecycle.market_exit_order_id = str(mid) if mid is not None else None
                            _log(f"[AUTOEXEC][BROKER] place_ok sym={lifecycle.symbol} lc={lifecycle.lifecycle_id} coid={mcoid} pid={mpid} oid={mid} leg=MKTP0N")
                        except Exception:
                            pass
                    _append_note(lifecycle, "TP0_TRIGGER_NOORDER_TIMEOUT_MARKET")
                    _mark_exit_sent(lifecycle, now, "TP0_TRIGGER_NOORDER_TIMEOUT_MARKET", order_id=lifecycle.market_exit_order_id, mode="market_close")
                    return

            # Once TP0 has triggered, keep trying to place/attach the TP0 limit order on subsequent reruns
            # even if the current last price has moved back below TP0.
            should_fire_tp0 = False
            ts = getattr(lifecycle, 'tp0_last_attempt_at', None)
            ok_attempt = True
            if ts:
                try:
                    dt = datetime.fromisoformat(str(ts))
                    ok_attempt = (now - dt).total_seconds() >= 10
                except Exception:
                    ok_attempt = True
            if tp0_px is not None and not lifecycle.tp0_order_id:
                if getattr(lifecycle, 'tp0_triggered_at', None):
                    should_fire_tp0 = True
                elif shared_last_px is not None and shared_last_px >= tp0_px and ok_attempt:
                    lifecycle.tp0_last_attempt_at = now.isoformat()
                    lifecycle.tp0_triggered_at = now.isoformat()
                    _append_note(lifecycle, f"TP0_TRIGGER_FIRED last={shared_last_px} tp0={tp0_px}")
                    should_fire_tp0 = True

            if should_fire_tp0 and tp0_px is not None and not lifecycle.tp0_order_id:
                stop_absent = _cancel_stop_and_confirm_absent(
                    ...
                    note_prefix="STOP_CANCEL_FOR_TP0_TRIGGER",
                )
                if not stop_absent:
                    _append_note(lifecycle, "TP0_WAITING_STOP_CANCEL_CONFIRM")
                else:
        ...
        # Only applies when TP0 has NOT already been triggered and current price is still below TP0.
        try:
            if bool(getattr(cfg, "enable_time_profit_capture", False)) and not getattr(lifecycle, "tp0_triggered_at", None) and not lifecycle.tp0_order_id:
                if tp0_px is None or shared_last_px is None or shared_last_px < tp0_px:
                    ...
                    if entry_dt is not None:
                        ...
                        if (now - entry_dt).total_seconds() >= mins * 60.0:
                            ...
                            if entry_px and entry_px > 0 and shared_last_px is not None:
                                ...
                                if gain_pct >= thresh:
                                    stop_absent = _cancel_stop_and_confirm_absent(
                                        ...
                                        lifecycle,
                                        filled_qty=bracket_qty,
                                        note_prefix="TIME_PROFIT_CAPTURE_STOP_CANCEL",
                                    )
                                    if not stop_absent:
                                        _append_note(lifecycle, "TIME_PROFIT_CAPTURE_WAITING_STOP_CANCEL_CONFIRM")
                                        return
                                    ...
                                    try:
                                        if rem_qty > 0:
                                            ...
                                            close_mode, close_oid, _ppid = _place_sell_close_best_effort(
                                                ...
                                                last_px=float(shared_last_px) if shared_last_px is not None else None,
                                                lifecycle=lifecycle,
                                    ...
                                    _mark_exit_sent(lifecycle, now, "TIME_PROFIT_CAPTURE", order_id=lifecycle.market_exit_order_id, mode=close_mode)
                                    return
        except Exception as e:
